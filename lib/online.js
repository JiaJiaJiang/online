...
class online extends EventEmitter {
	...
	handle(ws) {
		if (ws.readyState > 1) return;//只处理readyState为1的连接
		else if (ws.readyState === 0) {//如果还在建立连接的话等待连接建立并重新处理
			ws.on('open', ws => this.handle(ws));//等连接状态为打开时再重新进入这个过程
			return;
		}
		ws.on('message', data => this._wsMessage(ws, data));
		ws.on('ping', data => this._wsMessage(ws));
		ws.on('pong', data => this._wsMessage(ws));
		ws.aliveChecker = setInterval(() => {//每分钟进行一次连接可用性检查
			if (ws.lastActive < Date.now() - 30000) {//30秒内没有活动则发送一个ping
				ws.ping();
			} else if (ws.lastActive < Date.now() - this.timeout) {//超过30秒且在this.timeout内没有活动则尝试关闭连接
	...
	_wsMessage(ws, data) {
		...
		if (!data || data.byteLength === 0) return;//无视空包
		try {
 ...
 * @param {number} o	原进制
 * @param {number} t	目标进制
 * @param {string} [olist]	原数所用字符表
 * @param {string} [tlist]	目标字符表
 * @returns {string}  
 */
